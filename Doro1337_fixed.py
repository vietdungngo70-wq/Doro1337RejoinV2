...
class StatsManager:
    ...
    def save(self): with g_state["lock"]: json.dump(self.data, open("stats.json", "w"))
    def update(self, k): with g_state["lock"]: self.data[k] += 1
    def get_snapshot(self): with g_state["lock"]: return self.data.copy()
